...
func (r *inMemoryRepo) Create(i pomodoro.Interval) (pomodoro.IntervalID, error) {
	r.Lock()
	defer r.Unlock()
	i.ID = pomodoro.IntervalID(len(r.intervals)) + 1
...
func (r *inMemoryRepo) ByID(id pomodoro.IntervalID) (pomodoro.Interval, error) {
	r.RLock()
	defer r.RUnlock()
	i := pomodoro.Interval{}
	if id == 0 {
